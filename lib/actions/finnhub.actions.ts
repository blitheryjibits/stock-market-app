...
// create a query string type using template literal like `${string}?${string}`
...
export async function getStockQuote(
  ...
  try {
    ...
    if (!data || data.error) {
      console.error("Error fetching quote for", upper, data?.error);
      return null;
    }
...
export async function getStockQuotes(
  ...
  const validSymbols = Array.isArray(symbols)
    ? symbols.filter((s) => typeof s === "string" && s.trim())
    : [];
  if (validSymbols.length === 0) return {};

  try {
    const quotes = await Promise.all(
      validSymbols.map((sym) => getStockQuote(sym)),
    );
